...
function AppointmentSlotModal(props) {
    const { setShowModal, getDaySlots, day, editId, setEditId, edit, setEdit } = props
    ...
    function getEditSlot() {
        axiosBaseUrl.get(`private/appointment/slot-detail/${editId}`)
            .then((res) => {
                // console.log(moment(res.data.data[0].end_time, "hh:mm A").format("HH:mm"))
                // console.log(res.data.data[0].end_time)

                const newData = { ...slot }
                newData['total_beds'] = res.data.data[0].total_beds
                newData['start_time'] = moment(res.data.data[0].start_time, "hh:mm A").format("HH:mm")
                newData['end_time'] = moment(res.data.data[0].end_time, "hh:mm A").format("HH:mm")
    ...
    function updateSlot() {
        var  url = `private/appointment/slot/update/${editId}`
                ...
                <form onSubmit={submitCenterSlotForm}>
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalLabel">{editId ? 'Update' : 'Add'} Day Slot</h5>
                            <button type="button" className="close" data-dismiss="modal" aria-label="Close" onClick={() => {setShowModal(false);setEditId('')}}>
                        ...
                        <div className="modal-footer">
                            <button type="button" className="btn btn-secondary" data-dismiss="modal" onClick={() => {setShowModal(false);setEditId('')}}>Close</button>
